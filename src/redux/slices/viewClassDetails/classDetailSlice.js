...
export const fetchTrainerTrainingClass = createAsyncThunk(
  ...
  async ({id, token}) => {
    const baseUrl = `https://f-m-c-v3.azurewebsites.net/api/user/all-class-trainers?id=${id}`;
    // API
    const queryUrl = new URL(baseUrl);
...
export const fetchAdminTraining = createAsyncThunk(
  ...
  async ({id, token}) => {
    const baseUrl = `https://f-m-c-v3.azurewebsites.net/api/user/all-class-admins?id=${id}`;
    // API
    const queryUrl = new URL(baseUrl);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTrainerTrainingClass.fulfilled, (state, action) => {
        state.loading = false;
        state.trainingTrainerClassdata = action.payload;
      })
