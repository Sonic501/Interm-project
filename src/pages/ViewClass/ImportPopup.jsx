...
import { uploadTrainingClass } from '../../redux/slices/viewClass/uploadSlice';
...
const ImportPopup = (props) => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await axios({
        url: 'https://f-m-c-v3.azurewebsites.net/api/file-csv/download/class-format',
        method: 'GET',
      ...
      link.href = url;
      link.setAttribute('download', 'class-format.csv');
  ...
  function handleUpload() {
    dispatch(
      uploadTrainingClass({file, token})
    );
  }
